...
| A spawned program reaching outside the workspace on its own | Path confinement binds the agent's *tools*; a subprocess resolves paths however it likes. npm's rule is to search upwards for a `package.json`, so `npm install` in a workspace that has none climbs out of it — observed live, installing a dependency into a project three directories above anything the user had opened, exit code 0. `scriptPreflight` now resolves that search itself and refuses any package-manager command whose manifest would land outside the workspace. The general point stands and is not fixable in a path guard: an approved command runs with the user's own privileges, which is what the approval is *for*. |
