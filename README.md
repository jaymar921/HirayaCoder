...
- 💬 **It knows when you're talking to it** — say hello, ask what model it is, or ask what you discussed earlier, and you get an answer instead of a file-reading loop. Agent mode classifies each message first and switches itself; the moment you ask for work, every tool is back. No button to remember.
...
- 🧠 **Smarter small models via memory** — three layers, all local. A plain-text session log (`.hirayacoder/memory/`) of what the agent did, the conversation itself, and typed facts about the project (`.hirayacoder/facts.jsonl`) — a missing toolchain, a decision you made — that persist across every session in the workspace, so the second session does not rediscover what the first one paid for.
- ✅ **"Done" has to be true** — a run that reports it finished while having written nothing, or having left `// Implement this here` inside a function it just wrote, gets sent back once with the specific problem named.
