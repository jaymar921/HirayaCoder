...
- `write_file`, `delete_file`, `create_folder`, `delete_folder`, and `run_script` all route through `security/permissionGate.js` and respect the current `permissionModes.js` state exactly like the Tier A native tool-calling loop — the ReAct loop does not bypass approval, it only changes how the *decision* to act is produced.
...
- This prompt (and the mutating actions in its schema — `write_file`, `delete_file`, `create_folder`, `delete_folder`, `run_script`, `run_tests`) is only used when the chat tab's mode button is set to **Agent**. In **Plan** mode, the same loop runs with every mutating action removed from the schema shown to the model, and the run ends by rendering a checklist instead of executing anything. In **Ask** mode, this prompt isn't used at all — no loop runs.
